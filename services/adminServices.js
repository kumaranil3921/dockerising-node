...
const getAdmin = function (criteria, projection, options, callback) {
	Models.Admins.find(criteria, projection, options, function (err, result) {
		if (err) {
...
const createAdmin = function (objToSave, callback) {
	new Models.Admins(objToSave).save(function (err, data) {
		if (err) {
...
const updateAdmin = function (criteria, dataToSet, options, callback) {
	options.lean = true;
	options.new = true;
	Models.Admins.findOneAndUpdate(criteria, dataToSet, options, function (err, data) {
		if (err) {
			if (err.name == "CastError") {
				return callback(Confi);
			}
			if (err.code == 11000 && err.message.indexOf('phoneNo_1') > - 1) {
